...
class VGG11(nn.Module):
    def __init__(self):
        ...
        self.features = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=3, padding=1),nn.GroupNorm(1,64), nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(64, 128, kernel_size=3, padding=1),nn.GroupNorm(1,128), nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(128, 256, kernel_size=3, padding=1),nn.GroupNorm(1,256), nn.ReLU(inplace=True),
            nn.Conv2d(256, 256, kernel_size=3, padding=1),nn.GroupNorm(1,256), nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(256, 512, kernel_size=3, padding=1),nn.GroupNorm(1,512), nn.ReLU(inplace=True),
            nn.Conv2d(512, 512, kernel_size=3, padding=1),nn.GroupNorm(1,512), nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(512, 512, kernel_size=3, padding=1),nn.GroupNorm(1,512), nn.ReLU(inplace=True),
            nn.Conv2d(512, 512, kernel_size=3, padding=1),nn.GroupNorm(1,512), nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2),
...
def main():
    ...
    with open('results_groupnormg=1.csv', mode='w', newline='') as file:
