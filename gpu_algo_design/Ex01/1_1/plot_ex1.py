...
for (transfer_type, operation), group in df_host_trans.groupby(['Transfer Type', 'Operation']):
    axes[0].scatter(group['Size (bytes)'], group['Bandwidth (GB/s)'], alpha=0.7, label=f'{transfer_type}-{operation}')
for (transfer_type, operation), group in df_dev_trans.groupby(['Transfer Type', 'Operation']):
    axes[1].scatter(group['Size (bytes)'], group['Bandwidth (GB/s)'], alpha=0.7, label=f'{transfer_type}-{operation}')
...
plt.savefig('thrp_ex1.pdf', format='pdf')
